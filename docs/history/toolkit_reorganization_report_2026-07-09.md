...
历史范围：这是当日机器 `/overflow/htzhu/mingcheng_new/AI_Research_Toolkit` 的重组记录，不代表其他机器或当前环境的事实来源。最新状态请运行 `bin/ai-research-toolkit status`、`bin/ai-research-toolkit doctor` 和 `bin/ai-research-toolkit smoke`。
...
- Mermaid：`mmdc` 已安装，但当前节点的 Chromium 启动失败，错误来自 crashpad/socket 权限限制；详见 `docs/local_install_report.md`
- PlantUML：生成 `smoke/plantuml/smoke_sequence.svg`
- Typst：生成 `smoke/typst/smoke_note.pdf`
- Manim：只做版本检查，不渲染长视频
...
当前 core/recommended 检查项中，Mermaid CLI、PlantUML、Typst、Manim、diagrams.py 都已安装到本仓库本地环境并通过版本检查。仍需后续处理：

- Mermaid CLI 的 Chromium headless 渲染环境限制。
